...
public class RedirectController {
    ...
    public void serveIndex(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String HEROKU_URL = dotenv.get("HEROKU_URL");
        String DOMAIN_URL = dotenv.get("DOMAIN_URL");
        String LOCALHOST_URL = dotenv.get("LOCALHOST_URL");
        String LOCALHOST_PORT = dotenv.get("LOCALHOST_PORT");
        ...
        if (hasRedirected1 == null || !hasRedirected1) {
            if (url != null && !url.contains("https://" + DOMAIN_URL)) {
        ...
        if (hasRedirected1 == null || !hasRedirected1) {
           ...
           if (host != null && host.contains(LOCALHOST_URL) || url != null && url.contains(LOCALHOST_URL)) {
               session.setAttribute("hasRedirected1", true);
               response.sendRedirect("http://" + LOCALHOST_URL + ":" + LOCALHOST_PORT);
            }   
           if (host != null && host.contains("herokuapp") || url != null && url.contains("herokuapp")) {
               session.setAttribute("hasRedirected1", true);
               response.sendRedirect("https://" + HEROKU_URL);
            }   
           if (host != null && host.contains(DOMAIN_URL) || url != null && url.contains(DOMAIN_URL)) {
               session.setAttribute("hasRedirected1", true);
               response.sendRedirect("https://" + DOMAIN_URL);
            } else                      
