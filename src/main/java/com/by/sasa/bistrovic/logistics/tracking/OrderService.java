...
public class OrderService {
    ...
    public Order createOrder(Order orderData) {
        ...
        statusUpdate.setNote("Order created with status: accepted");
        ...
        statusUpdate2.setStatus("accepted");
        statusUpdate2.setTimestamp(java.time.LocalDateTime.now().toString());
        statusUpdate2.setNote("Order automatically accepted with price: " + orderData.getPrice() + " " + orderData.getCurrency());
...
public Order updateOrderStatus(String orderId, StatusUpdateRequest request) {
    ...
    List<Order> findOrderByTransporterVehicleId(String transporterVehicleId) {
        return orderRepository.findByTransporterVehicleIdAndStatusIn(transporterVehicleId, List.of(OrderStatus.determine_price, OrderStatus.accepted, OrderStatus.pickup, OrderStatus.in_transit));
    }
}
