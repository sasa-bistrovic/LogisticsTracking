...
export default function EditVehicleScreen() {
  ...
  const { userTest, updateUserTest, updateLoginVehicleTest } = useAuthStore2();
  
  const { fetchOrders, isLoading } = useOrderStore2();
  ...
  useEffect(() => {
    if (userTest?.vehicles) {
      const foundVehicle = userTest.vehicles[0];
      if (foundVehicle) {
...
const updatedVehicles = userTest.vehicles?.map(v => 
...
updateUserTest(updatedUser);
      updateLoginVehicleTest(updatedVehicle);
