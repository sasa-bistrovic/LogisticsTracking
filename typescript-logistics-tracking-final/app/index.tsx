...
export default function WelcomeScreen() {
  ...
  // Funkcija koja se pokreće kad god se vratiš na / i sesija traži welcome
  const checkAndRedirect = () => {
    /*
    const path = sessionStorage.getItem("path");
    const from = params.from;
      console.log(path)
    if (pathname === '/' && path === "welcome" && from !== "main") {
      console.log("YessM");
      //sessionStorage.removeItem("path");
      sessionStorage.setItem("path2", "welcome2");
      sessionStorage.setItem("path", "welcome2");
  ...
  useEffect(() => {
    ...
    // Presretni back/forward klikove
    const handlePopState = () => {
      checkAndRedirect();
    };

    // Ako user npr. klikne iz browser tab history-a
  ...
  return (
    ...
    <>
      <Head>
        ...
        />
        <link rel="icon" href="https://logitrack.expense-tracking.com/favicon.ico" type="image/x-icon" />
        <link rel="shortcut icon" href="https://logitrack.expense-tracking.com/favicon.ico" type="image/x-icon" />

        <link rel="canonical" href="https://logitrack.expense-tracking.com" />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "WebSite",
              "name": "LogiTrack",
              "url": "https://logitrack.expense-tracking.com",
              "description":
